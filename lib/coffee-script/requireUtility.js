...
(function() {
  ...
  module.exports = {
    importRequires: function(localContext, workingDir) {
      var comments, contextName, e, fileNames, finalRequireFailures, functionDatums, functionName, internalRequires, manualHash, params, remaining, requireIters, swagg, uniqInternals, _ref, _ref1, _ref2;
      ...
      if (uniqInternals.length > 0) {
        _.each(uniqInternals, function(ui) {
          var e;
          ui[0] = ui[0].replace("/server", "/dist/server");
          try {
            localContext[ui[1]] = eval("require('" + ui[0] + "')");
            return console.log("Loaded: " + ui);
          } catch (_error) {
            e = _error;
            return finalRequireFailures.push(ui);
          }
        });
        console.log("" + finalRequireFailures.length + " Modules failed to load: ");
        console.log((_.collect(finalRequireFailures, function(ui) {
          return ui[1];
        })).join(","));
      }
      ...
      for (contextName in manualHash) {
        ...
        _ref2 = functionDatums.comments;
        for (functionName in _ref2) {
          comments = _ref2[functionName];
          try {
      ...
      localContext.cmnt = function(obj) {
        var cmnt, _cmnt;
        cmnt = obj.__cmnt__;
        if (cmnt != null) {
          if (typeof cmnt === 'string') {
            ...
            if (typeof cmnt === 'object') {
              for (functionName in cmnt) {
                _cmnt = cmnt[functionName];
                console.log("" + functionName + "\n-------------");
                if ((_cmnt == null) || (_cmnt === "")) {
        ...
        if (obj.__man__ != null) {
          ...
          if (typeof data === 'string') {
            ...
            if (typeof data === 'object') {
              _ref3 = obj.__man__;
              for (functionName in _ref3) {
                params = _ref3[functionName];
    ...
    ensureWorkingDirIsServer: function(workingDir) {
      var checkLevel, currentDirs, foundServerDir, max_level_check;
      ...
      if (_(currentDirs).contains("server")) {
        return "" + workingDir + "/server";
      }
      max_level_check = 3;
      checkLevel = 0;
      foundServerDir = false;
      while ((!foundServerDir) && (checkLevel < max_level_check)) {
        workingDir = workingDir.split("/").slice(0, -1).join("/");
